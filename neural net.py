...
def create_train_sample(train_data, train_labels):
    train_image_sample = np.ones((43360, 784))
    parity_train_sample = np.ones((2,43360))
    magnitude_train_sample = np.ones((2,43360))
    train_label = np.ones(43360)
    ...
    for current_number in range(1,9):
        ...
        while counter < len(train_labels):
            ...
            if(current_label == current_number):
                ...
                train_image_sample[sample_size] = np.reshape(train_data[counter], (1,784))
...
def shuffle_train_data(train_image_sample, parity_train_sample, magnitude_train_sample, train_label):
    train_image_sample_random = np.ones((43360, 784))
    train_label_random = np.ones(43360)
    parity_train_sample_random = np.ones((2,43360))
    magnitude_train_sample_random = np.ones((2,43360))
    ...
    return train_image_sample_random, train_label_random, parity_train_sample_random, magnitude_train_sample_random, index_locations
...
def read_MNIST_data(train_image_path, test_image_path, train_label_path, test_label_path):
...
def train_model(train_image_sample_random, parity_train_sample_random, magnitude_train_sample_random, epochs = 400):
    ...
    model.fit(train_image_sample_random, [parity_train_sample_random.T, magnitude_train_sample_random.T], epochs)
...
if __name__ == "__main__":
    ...
    train_data, train_labels, test_data, test_labels = create_train_sample(train_data, train_labels)

    train_image_sample_random, train_label_random, parity_train_sample_random, magnitude_train_sample_random, index_locations = shuffle_train_data(train_image_sample, parity_train_sample, magnitude_train_sample, train_label)


    model = train_model(train_image_sample_random, parity_train_sample_random, magnitude_train_sample_random)
    ...
    train_labels_new = np.ones(43360)
