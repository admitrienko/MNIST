...
def plot_MDS(input_dataset, input_labels, number_points = 800, title = "MDS Plot"):
    ...
    plt.scatter(train_image_transformed[:,0], train_image_transformed[:,1], c =colors[1200:1600])
    ...
    plt.legend(handles=[red_patch, pink_patch, blue_patch, navy_patch], loc = (1,0), prop={'size': 20})
...
def error_bar_plot(NN_data, NN_sd, TME_data, TME_sd, random_data, random_sd, metric = "CCGP"):

    sample_CCGP = np.load('/Users/anastasia/Desktop/randtensor/values/new_real_CCGP.npy')
    ...
    plt.errorbar(range(0,len(CCGP_means)),CCGP_means, yerr = 2*np.array(CCGP_sd), color = "black")

    plt.scatter(range(0,len(CFR_CCGP_means)),CFR_CCGP_means, color = "orange")
    plt.errorbar(range(0,len(CFR_CCGP_means)),CFR_CCGP_means, yerr = 2*np.array(CFR_CCGP_sd), color = "orange")
...
def get_PS(data, labels):
    ...
    for i in range(len(data)):
        ...
        for d in dichotomies:
            ...
            p_score_value = p_score(dataset, dichotomy = d, labels)
...
def get_CCGP(data, labels):
    ...
    for i in range(len(data)):
        ...
        for d in dichotomies:
            ...
            ccgp_value = ccgp(dataset, dichotomy = d, labels)
